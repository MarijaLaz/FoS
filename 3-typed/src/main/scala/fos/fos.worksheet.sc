...
reduce(parse("0 (iszero 0)", term).get)
...
typeof(List.empty, parse("(\\x:Bool.\\x:Nat.x)", term).get)
...
is_num(parse("succ succ succ 0", term).get)
is_val(parse("succ true", term).get)
is_val(parse("succ succ 0", term).get)
reduce(parse("fst {succ pred 0, false}", term).get)